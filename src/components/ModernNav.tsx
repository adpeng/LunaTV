...
export default function ModernNav() {
  ...
  const isActive = (href: string) => {
    ...
    return (
      ...
      (decodedActive.startsWith('/douban') && typeMatch && decodedActive.includes(`type=${typeMatch}`))
    );
  };
  ...
  return (
    <>
                  ...
                  >
                    ...
                    <span
                      className={`text-sm font-medium transition-all duration-300 ${
                        isItemActive ? `${item.color} font-semibold` : 'text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-gray-100'
                      }`}
                    >
                      {item.label}
                    </span>
      ...
      {showMoreMenu && (
        <div className="md:hidden fixed inset-0 bg-black/40 backdrop-blur-sm" style={{ zIndex: 2147483647 }} onClick={() => setShowMoreMenu(false)}>
          <div className="absolute bottom-20 left-2 right-2 bg-white/90 dark:bg-gray-900/90 backdrop-blur-3xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-800/30 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200/50 dark:border-gray-700/50">
              ...
              <button onClick={() => setShowMoreMenu(false)} className="p-2 rounded-full hover:bg-gray-200/50 dark:hover:bg-gray-700/50 transition-colors">
            ...
            <div className="grid grid-cols-4 gap-4 p-4">
              {menuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <div className={`flex items-center justify-center w-12 h-12 rounded-2xl ${isItemActive ? `bg-gradient-to-br ${item.gradient}` : 'bg-gray-100 dark:bg-gray-800'}`}>
                      <Icon className={`w-6 h-6 ${isItemActive ? 'text-white' : 'text-gray-600 dark:text-gray-400'}`} />
                    </div>
                    <span className={`text-xs font-medium ${isItemActive ? item.color : 'text-gray-700 dark:text-gray-300'}`}>{item.label}</span>
                  </Link>
                );
              })}
            </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        <div className="mx-2 mb-2 rounded-[28px] bg-white/70 dark:bg-gray-900/70 backdrop-blur-3xl shadow-2xl border border-white/20 dark:border-gray-800/30 overflow-hidden">
          <div className="h-[2px] bg-gradient-to-r from-transparent via-green-500/50 to-transparent" />
          ...
          <div className="flex items-center justify-around px-2 py-2 h-16">
            {menuItems.slice(0, 4).map((item) => {
              ...
              return (
                ...
                >
                  {isItemActive && <div className={`absolute -inset-2 bg-gradient-to-br ${item.gradient} opacity-15 rounded-2xl blur-sm`} />}

                  <div className={`relative flex items-center justify-center w-11 h-7 rounded-2xl transition-all duration-300 ${isItemActive ? 'bg-gradient-to-br from-gray-100/80 to-gray-200/60 dark:from-gray-800/80 dark:to-gray-700/60 shadow-lg' : 'bg-transparent'}`}>
                    <Icon className={`w-5 h-5 transition-all duration-300 ${isItemActive ? item.color : 'text-gray-600 dark:text-gray-400'}`} strokeWidth={isItemActive ? 2.5 : 2} />
                  </div>

                  {isItemActive && <span className={`text-[10px] font-semibold ${item.color} transition-all duration-300`}>{item.label}</span>}

                  {isItemActive && <div className={`absolute -bottom-0.5 w-1 h-1 bg-gradient-to-r ${item.gradient} rounded-full shadow-lg`} />}
                </Link>
              );
            })}

            {/* More button */}
            <button onClick={() => setShowMoreMenu(true)} className="relative flex flex-col items-center justify-center gap-0.5 min-w-[56px] transition-all duration-300 active:scale-95">
